...
export default function MainApp({ user, initialFavorites }: { user: any; initialFavorites: any[] }) {
  ...
  async function fetchPoint(lat: number, lng: number, name?: string) {
    ...
    try {
      ...
      const score = calcScore(
        ...
        h, 0.15, marineResult.waveHeight, pressNow, pressureTrend,
        marineResult.seaTemp, precip, moon.score, marineResult.currentSpeed, vis
      )
  ...
  useEffect(() => {
    ...
    import('leaflet').then(L => {
      ...
      const seaBase = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '© OpenStreetMap', maxZoom: 18,
      })
      ...
      seaLayerRef.current = { base: seaBase, marks: seaMarks }
      mapRef.current = map
            ...
            <div className="p-4 space-y-3 flex-1">
              ...
              {activeTab === 'tidevann' && (
                <div className="space-y-3">
                  ...
                  <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-2">Beste tidspunkter</p>
                  <div className="space-y-1">
                    {Array.from({ length: 24 }, (_, h) => ({ h, score: scoreTide(h) }))
                      .filter(x => x.h >= 4 && x.score >= 75)
                  ...
                  <div className="bg-blue-50 rounded-xl p-3">
                    ...
                    <p className="text-xs text-blue-600">Stigende tidevann mot høyvann er generelt best. Fisken er mer aktiv og beveger seg innover. Unnga lavvann der det er mulig.</p>
                  </div>
